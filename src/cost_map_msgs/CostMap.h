#ifndef _ROS_cost_map_msgs_CostMap_h
#define _ROS_cost_map_msgs_CostMap_h
  ...
  class CostMap : public ros::Msg
  {
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(data_lengthT > data_length)
        this->data = (std_msgs::UInt8MultiArray*)realloc(this->data, data_lengthT * sizeof(std_msgs::UInt8MultiArray));
      data_length = data_lengthT;
      for( uint32_t i = 0; i < data_length; i++){
      offset += this->st_data.deserialize(inbuffer + offset);
        memcpy( &(this->data[i]), &(this->st_data), sizeof(std_msgs::UInt8MultiArray));
      }
    ...
    const char * getType(){ return "cost_map_msgs/CostMap"; };
    const char * getMD5(){ return "583e670cabf60540675068c5ddc7575c"; };
